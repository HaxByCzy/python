...
def coinUseNum(inputTime, outputFile):
    ...
    bdwzNum = []
    #赠币消耗量
    sql = "SELECT topic_id, sum(inverse_num) as used, COUNT(DISTINCT(user_id)) as users from comic_order_inverse where (topic_id = " + str(hdsId) + " or topic_id = " + str(bdwzId) + " or topic_id = " + str(cqytId) + ") and pay_type = -1 GROUP BY topic_id;"
    cur.execute(sql)
    res = cur.fetchall()
    if len(res) > 0:
        for row in res:
    ...
    if len(bdwzNum) == 0:
        bdwzNum = [0, 0, 0 ,0]
    print len(hdsNum),hdsNum
    print len(cqytNum),cqytNum
    print len(bdwzNum),bdwzNum
