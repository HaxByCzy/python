...
class Reducer(object):
    ...
    def outputSingleTopicResult(self):
        ...
        """
        if self.tmpDict:
            if "half" in self.tmpDict:
                if "year" in self.tmpDict:
                    sys.stdout.write("{0}\t{1}\n".format(self.currentKey, self.tmpDict["year"]))
                    sys.stderr.write("reporter:counter:weibo,halfTopicUid,1\n")
            else:
                sys.stderr.write("reporter:counter:weibo,restTopicUid,1\n")
