...
# 对选出的特征建立索引，以生成训练数据集
def featureIndex():
    outFileName = "featureMiTIndex.dat"
    featureSer = Series({})
    ageTypeList = np.array(["0", "1", "2", "3", "4", "5", "6", "7"])
    ageTypeNum = Series({"0" : 19157.0,"1" : 25989.0, "2" : 34397.0, "3" : 14747.0, "4" : 8241.0, "5" : 3671.0, "6" : 1641.0, "7" : 3305.0})
    print "mutual Infor..."
    ...
    for ageType in ageTypeList:
        ...
        for key, val in oneTypeSer.iteritems():
            if total < ageTypeNum[ageType] * 0.28:
                featureSer[key] = 1
                total += 1
...
def mutualInfor():
    ...
    ageTypeNum = Series({"0" : 99934.0,"1" : 377981.0, "2" : 396867.0, "3" : 37225.0, "4" : 9226.0, "5" : 1693.0, "6" : 565.0, "7" : 4259.0})
    for ageType in ageTypeNum.index :
        oneAgeSer = Series.from_csv(path = "D://data//app" + ageType + ".dat", sep = "\t") 
        oneAgeSer = oneAgeSer / ageTypeNum[ageType]
...
def oneAgeAppFeature():
    ...
    rawDf = pd.read_table("D://data//uidAgeAppMerge.dat", sep = "\t", names = colNames)
    ...
    for ageType in ageTypes :
         ...
         with open("D://data//app" + str(ageType) + ".dat", "w") as outFile:
