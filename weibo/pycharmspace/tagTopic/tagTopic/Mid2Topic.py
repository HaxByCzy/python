            ...
            for elem in tagTopicArr:
                if elem:
                    ...
                    for topic in elemArr:
                        if topic and topic not in topicSet:
                            topicSet.add(topic)
                            topicLine += "{0}==++==baoquan3".format(topic)
            topicLine = topicLine.strip("==++==baoquan3")
            sys.stdout.write("{0}\t{1}\n".format(mid,topicLine ))
            sys.stderr.write("reporter:counter:weibo,hasTopicId,1\n")
        else:
            sys.stdout.write("{0}\t{1}\n".format(mid, "noTopicMid"))
            sys.stderr.write("reporter:counter:weibo,noTopicId,1\n")
